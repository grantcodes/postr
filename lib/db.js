  ...
  await db.collection({
    ...
    schema: {
      ...
      properties: {
        ...
        // TODO: Waiting on bug fix to get indexing to work https://github.com/pubkey/rxdb/issues/697
        properties: {
          type: 'object',
        },
        indexDate: {
    ...
    methods: {
      ...
      getReferences: function() {
        const doc = this
        let refs = {}
        const refsArray = doc.get('_refs') || []
        refsArray.forEach(ref => {
          refs.properties.url.forEach(url => {
            refs.url = ref
          })
        })
        return refs
            ...
            if (post.properties[key]) {
              post.properties[key].forEach(photo => {
                if (
                  ...
                  const photoFileLoc =
                    config.get('mediaDir') +
                    photo.replace(config.get('mediaBaseUrl'), '')
                  let photoObject = {}
      ...
      toMf2: function() {
        ...
        post.imageSizes = doc.getImageSizes()
