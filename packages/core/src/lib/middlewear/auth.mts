...
import { get, set } from '../config.mjs'
...
export default (req: Request, res: Response, next: NextFunction): void => {
  ...
  // Use a dangerousPermanentToken option to bypass token endpoint.
  if (get('dangerousPermanentToken') && get('dangerousPermanentToken') === token) {
    set('token', {
      me: get('siteBaseUrl'),
      scope: 'post create delete update',
      client_id: 'dangerousPermanentToken',
    })
    return next()
  }
  ...
  const fetchOptions = {
    ...
    headers: {
      ...
      'User-Agent': get('endpointBaseUrl'),
    },
  }

  // Verify the token with the token endpoint
  fetch(get('tokenEndpoint'), fetchOptions)
    .then(response => {
      if (!response.ok) {
    ...
    .then(body => {
      ...
      if (
        new URL(tokenResponse.me as string).href !==
        new URL(get('siteBaseUrl') as string).href
      ) {
        return Promise.reject({
